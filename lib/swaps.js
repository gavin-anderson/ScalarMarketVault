...
async function swap041(inputAmount, signer, provider, addresses) {

    // Initialize contracts with the local signer
    const poolContract = new ethers.Contract(addresses.POOL_ADDRESS, IUniswapV3PoolABI, provider);
    const swapRouterContract = new ethers.Contract(addresses.SWAP_ROUTER_ADDRESS, SwapRouterABI, provider);
    ...
    await tokenContract.connect(signer).approve(addresses.SWAP_ROUTER_ADDRESS, approvalAmount);
    ...
    return {
        ...
        receipt: receipt,
        swappedDirection: _token0 == addresses.LONG_TOKEN_ADDRESS ? 'Long for Short' : 'Short for Long'

    };
}

async function swap140(inputAmount, signer, provider, addresses) {
    // Initialize contracts with the local signer
    const poolContract = new ethers.Contract(addresses.POOL_ADDRESS, IUniswapV3PoolABI, provider);
    const swapRouterContract = new ethers.Contract(addresses.SWAP_ROUTER_ADDRESS, SwapRouterABI, provider);
    ...
    return {
        transaction: transaction,
        receipt: receipt,
        swappedDirection: _token0 == addresses.LONG_TOKEN_ADDRESS ? 'Short for Long' : 'Long for Short'

    };
}

async function quote041(inputAmount, signer, provider, addresses) {

    [_token0, _token1] = await checkTokenHexOrder(addresses.LONG_TOKEN_ADDRESS, addresses.SHORT_TOKEN_ADDRESS);

    const poolContract = new ethers.Contract(addresses.POOL_ADDRESS, IUniswapV3PoolABI, provider);
    const { sqrtPriceX96 } = await poolContract.slot0();
    const QuoterContract = new ethers.Contract(addresses.QUOTERV2_ADDRESS, QuoterV2, provider);
...
async function quote140(inputAmount, signer, provider, addresses) {
